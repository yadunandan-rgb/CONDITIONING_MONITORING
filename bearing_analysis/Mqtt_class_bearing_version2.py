...
##fftavg=averaged fft values ####samplingFrequency of the sensor ###window size of the data.
    ...
    def INNERRACE_analysis(self,range1,range2,frequency): #checks whether innerrace therotical frequencies  range matches the the frqns greater then rms obtained from the parser
        ...
        try:
            ...
            print(str(e)) 
    def bearing_total(self,fftdata):
        try:
            ...
            fft_frqn=self.FFT_calculations(fftdata)
            bear_ftfresult=self.FTF_analysis(ftfrange2,ftfrange1,fft_frqn)
            bear_bsfresult=self.BSF_analysis(bsfrange2,bsfrange1,fft_frqn)
            bear_bpfiresult=self.INNERRACE_analysis(bpfirange2,bpfirange1,fft_frqn)
            bear_bpforesult=self.OUTERRACE_analysis(bpforange2,bpforange1,fft_frqn)
            return bear_ftfresult,bear_bsfresult,bear_bpfiresult,bear_bpforesult
        except Exception as e:
            print(str(e))