...
import Bearing_analysis_function_4 as fun
list1=[]
###input parameters ###
config_object = ConfigParser()
config_object.read(r'/home/vegam/bearinglatest2/userinfo6.ini')
...
####input data #############
basf=pd.read_csv(r'/home/vegam/bearinglatest2/bearingtesting/data/InnerRace_Fault.csv',skiprows=1,header=None)
########################
...
#    print(nwindows)
    ...
    a1,a2,a3,a4,a5,a6,a7,a8=fun.faultrange(r1,r2,r3,r4)
    ...
    ftf=fun.FTF(r1,r2,samplingFrequency,freqs1,afterallowance,afterfaultfreqallowanceminus,afterfaultfreqallowanceplus)
    if len(ftf) > 0:
        #####publishing the output
        print('strong_indication_for train frequency fault')
    
    ####frequencies belongs to ball spin frequency. 
    bsf=fun.BSF(r2,r3,samplingFrequency,freqs1,afterallowance,afterfaultfreqallowanceminus,afterfaultfreqallowanceplus)
    if len(bsf) > 0:
        #####publishing the output
        print("strong evidence for spin frequency fault")

    ####frequencies belongs to ball pass outer race frequency.         
    bpfo=fun.BPFO(r3,r4,samplingFrequency,freqs1,afterallowance,afterfaultfreqallowanceminus,afterfaultfreqallowanceplus)
    if len(bpfo) > 0:
        #####publishing the output
        print("strong evidence for outerrace fault")
    
    ####frequencies belongs to ball pass inner race frequency. 
    bpfi=fun.BPFI(r4,samplingFrequency,freqs1,afterallowance,afterfaultfreqallowanceminus,afterfaultfreqallowanceplus)
    if len(bpfi) > 0:
