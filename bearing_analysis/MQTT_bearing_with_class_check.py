...
while run_flag:
    ...
    if len(FFT_list) >= nwindows:
        ...
        bearing=fun.Bearing_analysis(NB,BD,PD,angle,shaftspeed,samplingFrequency,nsamp,windowsize,FFT_list)
        
        frqn=bearing.FFT_calculations() #using the fft caluclation function calculates the frequencies corresponding amplitudes greater than 3*rms
        #print("freqn is",freqs)
        
        #bearing2=fun.Bearing_analysis2(NB,BD,PD,angle,shaftspeed,samplingFrequency,nsamp,windowsize,FFT_list)
        f1=bearing.FTF_analysis(frqn) #checks whether ftf therotical frequencies  range matches the the frqns greater then rms obtained from the parser
        print(f1)
                #client.publish(pubishing_tag,f1)
        b1=bearing.BSF_analysis(frqn) #checks whether bsf therotical frequencies  range matches the the frqns greater then rms obtained from the parser
        print(b1) 
                #client.publish(pubishing_tag,b1)
        i1=bearing.INNERRACE_analysis(frqn) #checks whether innerace therotical frequencies  range matches the the frqns greater then rms obtained from the parser
        print(i1)
                #client.publish(pubishing_tag,i1)
        o1=bearing.OUTERRACE_analysis(frqn) #checks whether outerace therotical frequencies  range matches the the frqns greater then rms obtained from the parser
        print(o1)
        ...
        if len(FFT_list)>0:
            ...
            if time_diff >60:
                ...
                f1=bearing.FTF_analysis() #checks whether ftf therotical frequencies  range matches the the frqns greater then rms obtained from the parser
                print(f1)
                        #client.publish(pubishing_tag,f1)
                b1=bearing.BSF_analysis() #checks whether bsf therotical frequencies  range matches the the frqns greater then rms obtained from the parser
                print(b1) 
                        #client.publish(pubishing_tag,b1)
                i1=bearing.INNERRACE_analysis() #checks whether innerace therotical frequencies  range matches the the frqns greater then rms obtained from the parser
                print(i1)
                        #client.publish(pubishing_tag,i1)
                o1=bearing.OUTERRACE_analysis() #checks whether outerace therotical frequencies  range matches the the frqns greater then rms obtained from the parser
                print(o1)
