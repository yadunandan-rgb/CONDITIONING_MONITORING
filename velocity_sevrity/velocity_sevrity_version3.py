...
while run_flag:
    ...
    df = pd.DataFrame(list(queue),columns=['value','var','time_epoch']) #message (timestamp values) is converted to dataframe
    df=df.drop_duplicates(keep='first',inplace=False) #drop the duplicates values 
    ...
    if len(df)>=Window_size:
        ...
        #print(vel_value)
        timedataframe2=df['time'] #time is intervals are selected
        #print(timedataframe2)
        startandend_time=timedataframe2.iloc[[0, -1]] #amoung all window's date time only start time and end time is selected
        startandend_time= startandend_time.astype(str) #dataframe is converted as string
        print(startandend_time)
        v = startandend_time.to_numpy() #time is saved in form of numpy array
        list1.append(v) #start and endtime is saved in a list
        str1 = ','.join(str(e) for e in list1) #list is converted is to string
        #client.publish("ABC1234567/time_stamp",str1) #time at which fault is occured 
        client.publish("Tag_name2",Sevrity_level,str1)  #level of sevrity is published to vmaint using the mqtt topic 
