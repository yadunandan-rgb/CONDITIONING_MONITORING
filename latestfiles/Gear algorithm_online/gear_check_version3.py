...
import json
import numpy as np
import mqtt_gear_class_version2 as function
...
while run_flag:
    ...
    if len(FFT_list) >= nwindows:
        ...
        gear=function.GEAR_analysis(Speed_of_pinion,No_of_teeth_pinion,samplingFrequency,nsamp,windowsize,FFT_list)
        
        gear_result=gear.GEAR_total() #checks whether ftf therotical frequencies  range matches the the frqns greater then rms obtained from the parser
        print(gear_result)
                
        if len(gear_result) == 0: #checks whether list are empty 
