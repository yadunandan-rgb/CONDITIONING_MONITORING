...
import json
import numpy as np
import misalignment_function_with_class as function
...
while run_flag:
    ...
    if len(FFT_list) >= nwindows:
        result=function.Misalignment(samplingFrequency,nsamp,windowsize,FFT_list,reference_rpm)
        mis_result=result.Misalignment_total_check(FFT_list)
        print(mis_result)
