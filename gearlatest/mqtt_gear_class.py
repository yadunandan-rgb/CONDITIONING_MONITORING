...
##fftavg=averaged fft values ####samplingFrequency of the sensor ###window size of the data.
    ...
    def GMFFresult_analysis(self,frequency): #checks whether ftf therotical frequencies  range matches the the frqns greater then rms obtained from the parser
        self.corres_frequency=frequency
        global result
        for x in self.corres_frequency: 
            
            if x>= self.GMFFFinalrange2 and x<= self.GMFFFinalrange1: #first harmonic check
                result= "first harmonic FTF fault"
                
                self.resultant.append(result)
            if x>= 2*self.GMFFFinalrange2 and x<= 2*self.GMFFFinalrange1:##second harmonic check
                result="2nd harmonic ftf fault"
                #print("2nd ftf")
                self.resultant.append(result)

            if x>= 3*self.GMFFFinalrange2 and x<= 3*self.GMFFFinalrange1: ##third harmonic check
