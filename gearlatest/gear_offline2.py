# -*- coding: utf-8 -*-
"""
Created on Thu Nov  5 13:03:45 2020
...
config_object.read(r'D:\CONDITIONING_MONITORING-master (2)\CONDITIONING_MONITORING-master\gearlatest\gearboxinput.ini')
...
#frequency_of_pinion=float(Speed_of_pinion/60)
"""print(frequency_of_pinion)
GMFF=int(frequency_of_pinion*No_of_teeth_pinion)##355
print(GMFF)
GMFF2=int(2*GMFF)
GMFF3=int(3*GMFF)
...
GMFF2=gear.gear_calculation(No_of_teeth_pinion,frequency_of_pinion,No_of_teeth_gear,Sampling_freqn,Speed_of_pinion)    
GMFF3=gear.gear_calculation(No_of_teeth_pinion,frequency_of_pinion,No_of_teeth_gear,Sampling_freqn,Speed_of_pinion)    

###################
basf=pd.read_csv(r'D:\expirement\badffftparser.csv',skiprows=1,header=None)
########################
...
for row in range(0,len(basf),nwindows):
    ...
    for element in freqs:
        ...
        elif element in range(int(GMFF2-8),int(GMFF2+8)):
            print("their is fault in 2nd harmonic of gear")
        elif element in range(int(GMFF3-8),int(GMFF3+8)):
            print("their is 3rd fault in gear")
